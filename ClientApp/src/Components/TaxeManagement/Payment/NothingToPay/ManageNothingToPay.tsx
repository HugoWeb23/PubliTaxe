...
export const ManageNothingToPay = () => {
    ...
    const { entreprises, totalPages, pageCourante, elementsParPage, getAll, Insert } = useNothingToPay()
        ...
        <Container fluid="sm">
            ...
            <Table striped bordered hover size="sm">
                ...
                <tbody>
                    ...
                    {entreprises.map((notreceived: any, index: number) => <NotReceived element={notreceived} />)}
                </tbody>
            </Table>
...
const NotReceived = memo(({ element }: NotReceived) => {
    return <>
        <tr key={element.id_entreprise}>
            ...
            <td><Button size="sm" variant="success">Attribuer le statut rien à payer</Button></td>
        </tr>
        <tr>
            <td colSpan={5} className="px-3">
                <Table striped bordered hover size="sm" className="mt-1 mb-3">
                    <tr>
                        <th>N° panneau</th>
                        <th>Exonération</th>
                        <th>Taxe totale</th>
